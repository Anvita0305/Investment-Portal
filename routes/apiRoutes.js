...
router.get("/applications", jwtAuth, (req, res) => {
  ...
  Application.aggregate([
    ...
    {
      $match: {
        [user.type === "fund-finder" ? "recruiterId" : "userId"]: user._id,
      },
    },
    {
      $sort: {
...
router.get("/rating", jwtAuth, (req, res) => {
  ...
  Rating.findOne({
    ...
    category: user.type === "fund-finder" ? "investor" : "job",
  }).then((rating) => {
    if (rating === null) {
      res.json({
        rating: -1,
...
router.get("/investors", jwtAuth, (req, res) => {
  JobApplicant.find
  ({}).then((applicant) => {
